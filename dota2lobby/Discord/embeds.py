...
def LogEdit(message_before,message_after): #returns embed for Edit log
    embed = discord.Embed(title="{} edited his message in {} channel".format(str(message_before.author),
                                                                           str(message_before.channel.name)),
                          description="", color=discord.Color.from_rgb(255, 0, 100))
    if len(str(message_before.content)) < 256 and len(str(message_after.content)) <256:#adds pretty fields for small  embeds
...
def LogDelete(message,deleter): #returns embed for Delete log
    embed = discord.Embed(
        title="{} deleted a message of {} in {} text channel".format(str(deleter.name),str(message.author.name), str(message.channel.name)),
                        description="", color=discord.Color.from_rgb(255, 0, 0))
    if len(str(message.content)) < 256:
        embed.add_field(name=message.content, value="This is the message that he has deleted ", #adds pretty field for small  embed
                        inline = True)
    embed.add_field(name='Time: ', value= Kyiv_time())
    return embed
...
def LeaveLog(member): #returns embed for log of leaving members from channel
    embed=discord.Embed(title='{} leaved channel at {}'.format(str(member.name),Kyiv_time()),
                        color=discord.Color.from_rgb(0, 255, 0))
    return embed

def JoinLog(member): #returns embed for log of joining members from channel
    embed = discord.Embed(title='{} joins channel at {}'.format(str(member.name), Kyiv_time()),
